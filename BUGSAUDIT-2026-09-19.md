...
| `src/supervisor.*`, `startup_manager.*`, `runtime_state.*`, `command_queue.h`, `mingw_threading.h` | частично | скаут SidekickCore + личная охота за флейком command_queue | — |
...
Итоги: подтверждено 58 · гипотез 6 · областей закрыто 8 из 9 (не закрыта `keyboard-binding/` как не-продукт).
Правками закрыто: P0 — 1/1, P1 — 25 из 33, P2 — 9 из 24; остальное перечислено в §6 поимённо.
...
- **Severity:** P1 · **Статус:** подтверждено (scout ConfigModel; в sidekick.cpp комментарий это признаёт — проверено мной)
- **Последствие:** функция «запустить приложение, если окно не найдено» (`HandleMissingProfileTarget`) не может сработать после перезапуска: `rp.autoStart` всегда false; значение из файла затирается.
- **Не исправлено** — см. §6 (нужна правка схемы: `launchPolicy` ↔ `auto_start`).
...
- **Severity:** P1 · **Статус:** подтверждено (scout ConfigModel)
- **Последствие:** строки, которые парсер специально сохранял для совместимости, удаляются при сохранении из дашборда — «сохранённое» исчезает.
...
- **Severity:** P1 · **Статус:** подтверждено (scout ConfigModel)
- **Последствие:** битый/UTF-16 конфиг не откатывается на legacy-чтение, а деградирует к дефолтам и затем перезаписывается (данные теряются). ~170 строк `LoadConfigLegacy` недостижимы.
...
### [BA-33] `testConcurrentExactlyOnceCompletion` — флейк: 2/40 и 4/15 прогонов падают на `completionWins == 1`

- **Где:** `tests/command_queue_tests.cpp:212` (проверяемый модуль `src/command_queue.h`, в сборку `sidekick.exe` не входит)
- **Severity:** P2 (надёжность CI; модуль не в поставке) · **Статус:** подтверждено (измерено мной), корневая причина — гипотеза
- **Как заметить:** `./.ai-cache/command_queue.exe` в цикле — часть прогонов завершается `exit=3` с `Assertion failed ... completionWins.load() == 1`.
- **Замеры:** инструментированный вариант печатал в момент падения `state_after_pop=1 status_before=0 wins=0 status_after=0` (состояние так и осталось Pending, при этом все 16 вызовов `fulfillSuccess` вернули false). Логически это противоречит mutex-guarded `completeSuccess`, но под инструментацией (лишние проверки/вывод) падение не воспроизводится 0/60 — классический heisenbug.
- **Последствие:** периодически красный CI; продукт не затронут (класс не в сборке — это самостоятельная находка BA-32/§6).
- **Не исправлено:** см. §6.
...
- **Severity:** P2 · **Статус:** подтверждено (проверено мной)
- **Исправленный код:** формулировка приведена к коду. (Дополнительно: запуск `src\build.bat` из корня репозитория падает — `windres` получает `resources.rc` относительно CWD; README документирует `cd src`, поэтому оставлено как есть, но зафиксировано.)
- **Примечание:** в `README.ru.md`/`README.zh.md` та же фраза в переводе («Сначала ищет C:\MinGW64\bin\g++.exe…», «它首先查找…») — не правил, чтобы не трогать переводы без языковой вычитки (§6).
...
| `87f94eb` | BA-02…BA-17, BA-38(частично), BA-39, BA-40, BA-41 | `src/sidekick.cpp`, `src/report_diff.h` (новый), `tests/report_diff_tests.cpp` (новый), `run_all_tests.sh` | **новый тест до фикса-логики:** моя первая версия теста падала на шаге «удерживаемая клавиша не новый фронт» — тест чувствителен; **после:** `report_diff: 22 checks, 0 failed`; полный набор `15 passed, 0 failed` (дважды); интеграция `58 passed, 0 failed`; **запись конфига:** read-only `config.ini` → `HTTP 500`, вернув права → `HTTP 200 {"ok":true,...}` |
| `742202f` | BA-18, BA-19, BA-20, BA-21, BA-22, BA-23, BA-24, BA-42, BA-43, BA-44 | `src/sidekick.cpp`, `src/http_security.cpp`, `tests/http_security_tests.cpp`, `tests/runtime_storage_tests.cpp`, `web/app.js`, `run_all_tests.sh`, `dist/make_dist.ps1`, `README.md` | **XSS в браузере:** старая формула → `firedWithOldEscaping: true`; `jsStr` → `xssFired: false`, `actionsMatch: true`; **контракт detect:** `GET → 405`, `POST без токена → 403`, `POST с токеном → 200 {"detected":[]}`; **recursive-проверка:** `POST activate {"name":"nope-not-here"} → 404`; юнит-набор `15/15`; интеграция `58/58` |

Отклонение от плана скилла: правки разбиты на три коммита (P0; ввод+состояние/персистентность; безопасность+SSOT+тесты) вместо «одна группа — один коммит» — внутри групп файлы пересекаются (`sidekick.cpp`), поэтому раздельные коммиты по каждому багу требовали бы искусственного дробления и давали бы нерабочие промежуточные состояния.
...
| BA-28 | Требуется правка схемы: связать `ApplicationTarget.launchPolicy` с `auto_start` в bridge и проекции. Затрагивает config_v3/domain/bridge и фикстуры — отдельная работа (и потенциальная миграция конфигов). |
| BA-29 | То же: `Extension` не имеет представления в domain-модели; нужен носитель (или явный отказ от обещания «сохраняем неизвестные строки»). |
| BA-30 | Сделать `Parse` источником ERROR-диагностик — меняет поведение загрузки (и «оживляет» legacy-ветку), нужен отдельный проход с тестами на миграцию. |
...
| BA-32 | Судьба `StartupManager` (удалить как мёртвый код или подключить к продукту) — решение владельца; в аудите зафиксировано как «мёртвый модуль с тестом-заглушкой». |
| BA-33 | Не воспроизводится под инструментацией (0/60), модуль не в поставке; наугад править тест или очередь — подгонка. |
...
| README.ru.md / README.zh.md (BA-43) | Оставлены с прежней формулировкой о порядке тулчейна: правка переводов без носителя языка — хуже, чем зафиксированный дефект. |
...
| Показать красный CI на флейке `command_queue` | `run_all_tests.sh` теперь печатает вывод упавшего набора; воспроизведение: `for i in $(seq 1 40); do ./.ai-cache/command_queue.exe >/dev/null 2>&1 || echo fail; done` (~5–25 %) | отдельной сессии отладки (сборка с `-O0`/ThreadSanitizer-аналогом или Windows Application Verifier) |

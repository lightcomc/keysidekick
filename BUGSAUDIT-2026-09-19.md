...
Итоги: находок 68 · полностью подтверждено 66 · содержат непроверенную часть 2 (BA-26 — какие именно процессы не дают прочитать имя; BA-27 — переиспользование HWND ОС) · областей закрыто 8 из 9 (не закрыта `keyboard-binding/` как не-продукт).
Правками закрыто: P0 — 1/1, P1 — 30 из 36, P2 — 12 из 31; остальное перечислено в §6 поимённо.
...
### [BA-68] Ни одного флага предупреждений нигде и ни одного гейта в CI — статический анализ отсутствовал как практика

- **Где:** `src/build.bat:46` (`-O2 -fno-strict-aliasing` — без `-Wall`), `run_all_tests.sh` (сборка тестов без предупреждений), `.github/workflows/build.yml` (шага с предупреждениями нет); конфигов `clang-tidy`/`cppcheck`/`eslint` в репозитории нет
- **Severity:** P2 (инженерная практика, не дефект поведения) · **Статус:** подтверждено (проверено мной) · **Исправлено в `103f973`** (владелец: «ставь проверки») — гейт включён в сборку, раннер тестов и CI
- **Последствие (измеренное):** именно из-за отсутствия гейта в релиз уехали BA-65 (клавиша `;` не инжектилась — строка таблицы была съедена комментарием с обратным слэшем), BA-66 (падение на power-resume) и BA-67. Все три видны обычным `-Wall -Wextra`/`/analyze`.
- **Состояние после правок:** `g++ -Wall -Wextra` по 12 продакшн-файлам даёт только 123 `missing-field-initializers` (шум `= {0}` по Win32-структурам) — при подавлении этого класса вывод **пуст**; по 15 тестовым TU (с теми же стандартами, что использует раннер: `c++17` для `startup_manager`) — **ноль** предупреждений; MSVC `/analyze` по 12 продакшн-файлам даёт только 6 `C6246` (shadowing) и 1 `C6262` (24.8 КБ стека в HTTP-хендлере, информационно), классы C60xx/C63xx (неинициализированное, null-deref, переполнение буфера) — чисто.
- **Гейт поставлен:** `src/build.bat --check-warnings` (12 продакшн-единиц, exit 1 при любом warning/error), те же флаги в обычной сборке, те же флаги + проверка `*.build.log` в `run_all_tests.sh`, шаг «Warning gate» в CI.
- **Проверка фальсифицируемостью (три случая):** чистое дерево → `GATE_EXIT_CODE=0`; неиспользуемая локальная переменная → `warning: unused variable ...`, `GATE_EXIT_CODE=1`; **сломанный TU (синтаксическая ошибка) → `GATE_COMPILE_FAILED` + `error: ...`, `GATE_EXIT_CODE=1`** (это была реальная дыра: без проверки кода возврата каждой компиляции гейт рапортовал «OK» именно тогда, когда дерево не собирается). Плюс предупреждение в тестовом TU → раннер печатает `=== Test build warnings ===` и `16 passed, 1 failed — Failed: compiler-warnings`. После возврата файлов — снова чисто.
- **Повторная проверка (все четыре случая, обёртка с `call`):** чисто → `Warning gate OK`, `GATE_EXIT_CODE=0`; неиспользуемая переменная → `warning: unused variable ...`, `GATE_EXIT_CODE=1`; синтаксическая ошибка → `GATE_COMPILE_FAILED action_parser.cpp`, `GATE_EXIT_CODE=1`; подложенный старый `.ai-cache/*.build.log` → набор `16 passed, 0 failed` (лог удаляется в начале прогона).
- **Упавший гейт оставляет `src\warnings.log`** для разбора (в `.gitignore` он под `*.log`, дерево не пачкается); следующий запуск удаляет его в начале. Успешный — удаляет сразу.
- **Порядок в CI:** гейт стоит ПОСЛЕ сборки осознанно — `sidekick.cpp` включает генерируемый `dashboard_embedded.generated.h`, которого на чистом checkout нет; причина записана комментарием в workflow.
- **Измерение кодов возврата:** `%errorlevel%` в одной строке с `&` раскрывается до запуска и показывает 0 даже при срабатывании — проверять через обёртку с `call` и отдельной строкой `echo %errorlevel%` (зафиксировано в §5).
- **Грабли batch, на которые я наступил:** путь к логу нельзя хранить в переменной, заданной внутри блока в скобках (parse-time expansion) — первая версия гейта молча ничего не компилировала и рапортовала «OK».
- `clang-tidy`/`cppcheck`/`eslint` по-прежнему не установлены: первые два требуют пакета, третий — `package.json`; ставить не буду без явного согласия.
...
| `e5831f1` | BA-68 (упрочнение) | `src/build.bat`, `run_all_tests.sh`, `.github/workflows/build.yml` | **Три негативных контроля (обёртка, `%errorlevel%` отдельной строкой):** чисто → `Warning gate OK`, `GATE_EXIT_CODE=0`; неиспользуемая локальная переменная → `warning: unused variable ...`, `GATE_EXIT_CODE=1`; незакрытая скобка (дерево не компилируется) → `GATE_COMPILE_FAILED action_parser.cpp` + `error: expected ...`, `GATE_EXIT_CODE=1`. Закрыты две дыры ложного прохода: сломанный TU без `warning:` и нечитаемый лог. Подложенный старый `.ai-cache/*.build.log` с предупреждением больше не влияет (`16 passed, 0 failed`). Сборка OK, интеграция `58 passed / 0 failed` |
| `103f973` | BA-68 | `src/build.bat`, `run_all_tests.sh`, `.github/workflows/build.yml` | **Гейт поставлен и проверен на срабатывание:** чисто → `Warning gate OK`, exit 0; внесенная неиспользуемая переменная → `unused variable ... [-Wunused-variable]`, exit 1; предупреждение в тестовом TU → `16 passed, 1 failed — Failed: compiler-warnings`; после возврата — сборка OK, юнит 16/16, интеграция 58/58 |
| `0b05ab2`+ | BA-65, BA-66, BA-67, BA-68 | `src/sidekick.cpp`, `src/mingw_threading.h` | **BA-65:** `-Wcomment` исчез, строка `{0x33,0x27,false}` снова код; **BA-66/67:** MSVC `/analyze` больше не выдаёт C6387/C6053 ни по одному продакшн-файлу (осталось 6×C6246 shadowing + 1×C6262 стек); **BA-68:** прогон `-Wall -Wextra` по 12 продакшн- и 15 тестовым TU — тесты **0** предупреждений (4 каста `GetProcAddress` в шиме приведены к идиоме), продакшн — только `missing-field-initializers`; сборка OK, юнит 16/16, интеграция 58 passed / 0 failed |
| `0b05ab2` (исходный) | BA-65, BA-66, BA-67 | `src/sidekick.cpp` | **BA-65:** `-Wcomment` исчез, строка `{0x33,0x27,false}` снова код (до правки компилятор склеивал её с комментарием); **BA-66/67:** MSVC `/analyze` больше не выдаёт C6387 (NULL `ov.hEvent`/`g_hWinUsb`) и C6053 (`_snwprintf`); сборка OK, юнит 16/16, интеграция 58 passed / 0 failed |

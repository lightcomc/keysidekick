...
- **Проверка фальсифицируемостью:** чистое дерево → `GATE_EXIT_CODE=0`; внесённая неиспользуемая переменная → `warning: unused variable ...`, `GATE_EXIT_CODE=1`; внесённое предупреждение в тестовом TU → раннер печатает `=== Test build warnings ===` и `16 passed, 1 failed — Failed: compiler-warnings`. После возврата файлов — снова чисто.

...
Итоги: находок 64 · полностью подтверждено 62 · содержат непроверенную часть 2 (BA-26 — какие именно процессы не дают прочитать имя; BA-27 — переиспользование HWND ОС) · областей закрыто 8 из 9 (не закрыта `keyboard-binding/` как не-продукт).
Правками закрыто: P0 — 1/1, P1 — 28 из 34, P2 — 10 из 29; остальное перечислено в §6 поимённо.
...
| `a553a68`, `190a7cb` | BA-22 (класс), BA-57 | `tests/config_v3_tests.cpp` | **Позитив:** новый кейс разбирает реальный `src/config.example.ini` (≥1 профиль, без ошибок) — `config_v3` 7/7 из корня репозитория, прогон полного набора 16/16. **Ограничение (честно):** негативный контроль (спрятать файл и получить падение) в этой среде продемонстрировать не удалось — представление файловой системы у инструментов разошлось: harness-овые `mv`/`ls` показывали файл удалённым, тогда как нативный `dir` его видел, а одна правка исходника вообще не дошла до компилятора. Поэтому сам страж (`REQUIRE(exampleParsed)`) проверен по построению и по позитивному пути, а не наблюдённым падением. |
...
| Проверка «поставляемый пример валиден» (BA-22-класс) | **Сделано** в `a553a68`/`190a7cb`: кейс разбирает `src/config.example.ini`, а при отсутствии файла падает (`REQUIRE(exampleParsed)`), а не пропускает проверку. Остаток: негативный контроль не удалось показать в этой среде (расхождение представлений ФС у инструментов) — см. журнал правок. |

...
- **Severity:** P2 (инженерная практика, не дефект поведения) · **Статус:** подтверждено (проверено мной) · **Остаётся на решение владельца** (гейт не включён — см. §7)
...
- **Готовые команды** — в §7 (ничего доустанавливать не нужно). `clang-tidy`/`cppcheck` потребовали бы установки пакета, `eslint` — `package.json`; ни то, ни другое не ставил.

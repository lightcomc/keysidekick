...
- **Надёжность:**
  - Постоянный цикл устройства — приложение живёт, даже если клавиатура отключена
...
- **HTTP API** (только loopback, CSRF-защищённый):
  ...
  ```
  > **Для разработчиков:** `tests/http_integration_tests.sh` запускается против живого инстанса и **изменяет `src/config.ini`** — сначала сделайте бэкап.

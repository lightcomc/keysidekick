...
## Портативный режим (носи с собой на флешке)

KeySidekick полностью портативен: `sidekick.exe` хранит `config.ini` и логи **рядом с собой** (папка exe, с фолбэком на текущий каталог). Скопируй папку на флешку — профили, цели и маппинги поедут с ней. На другом компьютере:

1. Запусти `sidekick.exe` с флешки.
2. Открой дашборд → **+ Setup keyboard**: клавиатура в списке как обычная — один раз смени драйвер (**Swap automatically**, UAC-запрос появится только в этот момент) или выполни `sidekick.exe --driver swap vid_xxxx&pid_yyyy` напрямую.
3. Готово — профили для этого VID/PID уже в конфиге и применяются сразу. Перетыкание клавиатуры в другой USB-порт определяется автоматически с кнопкой «Apply driver again» в один клик.

Откат на любой машине: `sidekick.exe --driver restore vid_xxxx&pid_yyyy` (или Диспетчер устройств → удалить устройство → переподключить).

**Полностью офлайн.** KeySidekick не делает **ни одного сетевого соединения**: единственный сокет во всей программе — локальный дашборд на `127.0.0.1`. Никакой телеметрии, никакой облачной синхронизации, ничего не покидает машину. Облачные функции осознанно вне рамок проекта.
